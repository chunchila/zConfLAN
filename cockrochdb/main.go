...
func main() {
	...
	const addr = "postgresql://maxroach@52.226.79.38:26257/bank?sslmode=disable"
	...
	for index := 400; index < 1000; index++ {
		wg.Add(1)
		go func(index int, wg *sync.WaitGroup) {
			rand.Seed(time.Now().Unix())
			r := rand.Intn(123213)
			db.Create(&Account{ID: index, Balance: r})
			wg.Done()
		}(index, &wg)
	}

	log.Println("waitng for wg")
	wg.Wait()
	log.Println("finished waiting fo wg")
	log.Println("this is time since start ", time.Since(start))
	os.Exit(1)
