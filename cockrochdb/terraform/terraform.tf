...
provisioner "remote-exec" {
    inline = [
      ...
      "sudo killall cockroach",
      "sudo rm -rf cockroach-data",
      "sudo cockroach start --insecure  --advertise-host=${element(azurerm_network_interface.network_interface_deploy.*.private_ip_address,4)}  --join=${element(azurerm_network_interface.network_interface_deploy.*.private_ip_address,0)}:26257 --background",
    ]
    connection {
    user     = "azureuser"
...
provisioner "remote-exec" {
    inline = [
      ...
      "sudo killall cockroach",
      "sudo rm -rf cockroach-data",
      "sudo cockroach start --insecure  --advertise-host=${element(azurerm_network_interface.network_interface_deploy.*.private_ip_address,5)}  --join=${element(azurerm_network_interface.network_interface_deploy.*.private_ip_address,0)}:26257 --background",
    ]
    connection {
    user     = "azureuser"
